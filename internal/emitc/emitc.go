...
func (e *emitter) emitProgram(pkgs []*check.PkgInfo) (string, error) {
	...
	if e.needArena || e.needConcat || e.needAppend || e.needSlice || e.needMake || e.needRuneStr || e.needStrBytes || e.needMap || e.needDefer || e.needFunc || e.needChan || e.needGo {
		...
		b.WriteString("static void *aram_arena_alloc(size_t n) {\n")
		b.WriteString("\tif (aram_arena_len + n > aram_arena_cap) {\n")
		b.WriteString("\t\tsize_t nc = aram_arena_cap ? aram_arena_cap * 2 : 4096;\n")
		b.WriteString("\t\twhile (nc < aram_arena_len + n) nc *= 2;\n")
		b.WriteString("\t\taram_arena_buf = (unsigned char *)realloc(aram_arena_buf, nc);\n")
		b.WriteString("\t\tif (!aram_arena_buf) abort();\n")
		b.WriteString("\t\taram_arena_cap = nc;\n")
		b.WriteString("\t}\n")
		b.WriteString("\tvoid *p = aram_arena_buf + aram_arena_len;\n")
		b.WriteString("\taram_arena_len += n;\n")
		b.WriteString("\treturn p;\n")
		b.WriteString("}\n")
		b.WriteString("static void aram_arena_free(void) {\n")
		b.WriteString("\tfree(aram_arena_buf);\n")
...
func (e *emitter) writeFunc(b *strings.Builder, fn *ast.FuncDecl) {
	...
	if e.fnHasDefer {
		b.WriteString("\taram_defer_frame *_defers = NULL;\n")
		rts := e.resultTypes(fn)
...
func (e *emitter) writeDefer(b *strings.Builder, s *ast.DeferStmt, level int) {
	...
	tb.WriteByte('\t')
	e.writeDeferredCall(tb, call, builtinName, localFunc, pkgFunc, method, methodStruct, recvIsPtr, fields)
	tb.WriteString(";\n}\n")
