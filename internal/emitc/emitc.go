// Package emitc emits Aram programs as C99 source.
package emitc
...
func (e *emitter) emitProgram(pkgs []*check.PkgInfo) (string, error) {
	...
	b.WriteString("/* Generated by aram — do not edit by hand */\n")
	...
	b.WriteString("\n")
	b.WriteString("static void aram_print_int(int64_t v) { printf(\"%lld\\n\", (long long)v); }\n")
	b.WriteString("static void aram_print_float(double v) { printf(\"%g\\n\", v); }\n")
	b.WriteString("static void aram_print_str(const char *s) { printf(\"%s\\n\", s); }\n")
	b.WriteString("static void aram_print_bool(int v) { printf(\"%s\\n\", v ? \"மெய்\" : \"பொய்\"); }\n")
	b.WriteString("static void aram_print_int_n(int64_t v) { printf(\"%lld\", (long long)v); }\n")
	b.WriteString("static void aram_print_float_n(double v) { printf(\"%g\", v); }\n")
	b.WriteString("static void aram_print_str_n(const char *s) { printf(\"%s\", s ? s : \"\"); }\n")
	b.WriteString("static void aram_print_bool_n(int v) { printf(\"%s\", v ? \"மெய்\" : \"பொய்\"); }\n")
	b.WriteString("static void aram_print_quoted_n(const char *s) {\n")
	...
	b.WriteString("\tprintf(\"\\\"\");\n")
	b.WriteString("}\n")
	b.WriteString("static void aram_print_ptr_n(const void *p) {\n")
	...
	if e.needSlice || e.needAppend || e.needMake || e.needCopy {
		...
		b.WriteString("typedef struct { int *data; int64_t len; int64_t cap; } aram_slice_bool;\n")
		b.WriteString("typedef struct { const char **data; int64_t len; int64_t cap; } aram_slice_str;\n")
		b.WriteString("typedef struct { double *data; int64_t len; int64_t cap; } aram_slice_f64;\n")
		b.WriteString("typedef struct { uint8_t *data; int64_t len; int64_t cap; } aram_slice_u8;\n")
		b.WriteString("typedef struct { int32_t *data; int64_t len; int64_t cap; } aram_slice_i32;\n")
		b.WriteString("static int64_t aram_slice_get_i64(aram_slice_i64 s, int64_t i) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\treturn s.data[i];\n}\n")
		b.WriteString("static void aram_slice_set_i64(aram_slice_i64 s, int64_t i, int64_t v) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\ts.data[i] = v;\n}\n")
		b.WriteString("static int aram_slice_get_bool(aram_slice_bool s, int64_t i) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\treturn s.data[i];\n}\n")
		b.WriteString("static void aram_slice_set_bool(aram_slice_bool s, int64_t i, int v) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\ts.data[i] = v;\n}\n")
		b.WriteString("static const char *aram_slice_get_str(aram_slice_str s, int64_t i) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\treturn s.data[i];\n}\n")
		b.WriteString("static void aram_slice_set_str(aram_slice_str s, int64_t i, const char *v) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\ts.data[i] = v;\n}\n")
		b.WriteString("static double aram_slice_get_f64(aram_slice_f64 s, int64_t i) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\treturn s.data[i];\n}\n")
		b.WriteString("static void aram_slice_set_f64(aram_slice_f64 s, int64_t i, double v) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\ts.data[i] = v;\n}\n")
		b.WriteString("static uint8_t aram_slice_get_u8(aram_slice_u8 s, int64_t i) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\treturn s.data[i];\n}\n")
		b.WriteString("static void aram_slice_set_u8(aram_slice_u8 s, int64_t i, uint8_t v) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\ts.data[i] = v;\n}\n")
		b.WriteString("static int32_t aram_slice_get_i32(aram_slice_i32 s, int64_t i) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\treturn s.data[i];\n}\n")
		b.WriteString("static void aram_slice_set_i32(aram_slice_i32 s, int64_t i, int32_t v) {\n")
		b.WriteString("\tif (i < 0 || i >= s.len) abort();\n\ts.data[i] = v;\n}\n")
		b.WriteString("static aram_slice_i64 aram_sub_i64(aram_slice_i64 s, int64_t lo, int64_t hi) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || hi > s.len) abort();\n")
		b.WriteString("\treturn (aram_slice_i64){ s.data + lo, hi - lo, s.cap - lo };\n}\n")
		b.WriteString("static aram_slice_bool aram_sub_bool(aram_slice_bool s, int64_t lo, int64_t hi) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || hi > s.len) abort();\n")
		b.WriteString("\treturn (aram_slice_bool){ s.data + lo, hi - lo, s.cap - lo };\n}\n")
		b.WriteString("static aram_slice_str aram_sub_str(aram_slice_str s, int64_t lo, int64_t hi) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || hi > s.len) abort();\n")
		b.WriteString("\treturn (aram_slice_str){ s.data + lo, hi - lo, s.cap - lo };\n}\n")
		b.WriteString("static aram_slice_f64 aram_sub_f64(aram_slice_f64 s, int64_t lo, int64_t hi) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || hi > s.len) abort();\n")
		b.WriteString("\treturn (aram_slice_f64){ s.data + lo, hi - lo, s.cap - lo };\n}\n")
		b.WriteString("static aram_slice_u8 aram_sub_u8(aram_slice_u8 s, int64_t lo, int64_t hi) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || hi > s.len) abort();\n")
		b.WriteString("\treturn (aram_slice_u8){ s.data + lo, hi - lo, s.cap - lo };\n}\n")
		b.WriteString("static aram_slice_i32 aram_sub_i32(aram_slice_i32 s, int64_t lo, int64_t hi) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || hi > s.len) abort();\n")
		b.WriteString("\treturn (aram_slice_i32){ s.data + lo, hi - lo, s.cap - lo };\n}\n")
		b.WriteString("static aram_slice_i64 aram_sub3_i64(aram_slice_i64 s, int64_t lo, int64_t hi, int64_t max) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || max < hi || max > s.cap) abort();\n")
		b.WriteString("\treturn (aram_slice_i64){ s.data + lo, hi - lo, max - lo };\n}\n")
		b.WriteString("static aram_slice_bool aram_sub3_bool(aram_slice_bool s, int64_t lo, int64_t hi, int64_t max) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || max < hi || max > s.cap) abort();\n")
		b.WriteString("\treturn (aram_slice_bool){ s.data + lo, hi - lo, max - lo };\n}\n")
		b.WriteString("static aram_slice_str aram_sub3_str(aram_slice_str s, int64_t lo, int64_t hi, int64_t max) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || max < hi || max > s.cap) abort();\n")
		b.WriteString("\treturn (aram_slice_str){ s.data + lo, hi - lo, max - lo };\n}\n")
		b.WriteString("static aram_slice_f64 aram_sub3_f64(aram_slice_f64 s, int64_t lo, int64_t hi, int64_t max) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || max < hi || max > s.cap) abort();\n")
		b.WriteString("\treturn (aram_slice_f64){ s.data + lo, hi - lo, max - lo };\n}\n")
		b.WriteString("static aram_slice_u8 aram_sub3_u8(aram_slice_u8 s, int64_t lo, int64_t hi, int64_t max) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || max < hi || max > s.cap) abort();\n")
		b.WriteString("\treturn (aram_slice_u8){ s.data + lo, hi - lo, max - lo };\n}\n")
		b.WriteString("static aram_slice_i32 aram_sub3_i32(aram_slice_i32 s, int64_t lo, int64_t hi, int64_t max) {\n")
		b.WriteString("\tif (lo < 0 || hi < lo || max < hi || max > s.cap) abort();\n")
		b.WriteString("\treturn (aram_slice_i32){ s.data + lo, hi - lo, max - lo };\n}\n")
		b.WriteString("static const char *aram_sub_string(const char *s, int64_t lo, int64_t hi) {\n")
		b.WriteString("\tsize_t n = strlen(s);\n")
		b.WriteString("\tif (lo < 0 || hi < lo || (size_t)hi > n) abort();\n")
		b.WriteString("\tchar *r = (char *)aram_arena_alloc((size_t)(hi - lo) + 1);\n")
	...
	if e.needAppend {
		e.needArena = true
		b.WriteString("static aram_slice_i64 aram_append_i64(aram_slice_i64 s, int64_t v) {\n")
		...
		b.WriteString("\t\tint64_t *nd = (int64_t *)aram_arena_alloc((size_t)ncap * sizeof(int64_t));\n")
		b.WriteString("\t\tif (s.len && s.data) memcpy(nd, s.data, (size_t)s.len * sizeof(int64_t));\n")
		b.WriteString("\t\ts.data = nd; s.cap = ncap;\n")
		b.WriteString("\t}\n\ts.data[s.len++] = v;\n\treturn s;\n}\n")
		b.WriteString("static aram_slice_bool aram_append_bool(aram_slice_bool s, int v) {\n")
		b.WriteString("\tint64_t ncap = s.cap;\n")
		b.WriteString("\tif (s.len + 1 > ncap) {\n")
		b.WriteString("\t\tncap = ncap < 4 ? 4 : ncap * 2;\n")
		b.WriteString("\t\twhile (ncap < s.len + 1) ncap *= 2;\n")
		b.WriteString("\t\tint *nd = (int *)aram_arena_alloc((size_t)ncap * sizeof(int));\n")
		b.WriteString("\t\tif (s.len && s.data) memcpy(nd, s.data, (size_t)s.len * sizeof(int));\n")
		b.WriteString("\t\ts.data = nd; s.cap = ncap;\n")
		b.WriteString("\t}\n\ts.data[s.len++] = v;\n\treturn s;\n}\n")
		b.WriteString("static aram_slice_str aram_append_str(aram_slice_str s, const char *v) {\n")
		b.WriteString("\tint64_t ncap = s.cap;\n")
		b.WriteString("\tif (s.len + 1 > ncap) {\n")
		b.WriteString("\t\tncap = ncap < 4 ? 4 : ncap * 2;\n")
		b.WriteString("\t\twhile (ncap < s.len + 1) ncap *= 2;\n")
		b.WriteString("\t\tconst char **nd = (const char **)aram_arena_alloc((size_t)ncap * sizeof(const char *));\n")
		b.WriteString("\t\tif (s.len && s.data) memcpy(nd, s.data, (size_t)s.len * sizeof(const char *));\n")
		b.WriteString("\t\ts.data = nd; s.cap = ncap;\n")
		b.WriteString("\t}\n\ts.data[s.len++] = v;\n\treturn s;\n}\n")
		b.WriteString("static aram_slice_f64 aram_append_f64(aram_slice_f64 s, double v) {\n")
		b.WriteString("\tint64_t ncap = s.cap;\n")
		b.WriteString("\tif (s.len + 1 > ncap) {\n")
		b.WriteString("\t\tncap = ncap < 4 ? 4 : ncap * 2;\n")
		b.WriteString("\t\twhile (ncap < s.len + 1) ncap *= 2;\n")
		b.WriteString("\t\tdouble *nd = (double *)aram_arena_alloc((size_t)ncap * sizeof(double));\n")
		b.WriteString("\t\tif (s.len && s.data) memcpy(nd, s.data, (size_t)s.len * sizeof(double));\n")
		b.WriteString("\t\ts.data = nd; s.cap = ncap;\n")
		b.WriteString("\t}\n\ts.data[s.len++] = v;\n\treturn s;\n}\n")
		b.WriteString("static aram_slice_u8 aram_append_u8(aram_slice_u8 s, uint8_t v) {\n")
		b.WriteString("\tint64_t ncap = s.cap;\n")
		b.WriteString("\tif (s.len + 1 > ncap) {\n")
		b.WriteString("\t\tncap = ncap < 4 ? 4 : ncap * 2;\n")
		b.WriteString("\t\twhile (ncap < s.len + 1) ncap *= 2;\n")
		b.WriteString("\t\tuint8_t *nd = (uint8_t *)aram_arena_alloc((size_t)ncap * sizeof(uint8_t));\n")
		b.WriteString("\t\tif (s.len && s.data) memcpy(nd, s.data, (size_t)s.len * sizeof(uint8_t));\n")
		b.WriteString("\t\ts.data = nd; s.cap = ncap;\n")
		b.WriteString("\t}\n\ts.data[s.len++] = v;\n\treturn s;\n}\n")
		b.WriteString("static aram_slice_i32 aram_append_i32(aram_slice_i32 s, int32_t v) {\n")
		b.WriteString("\tint64_t ncap = s.cap;\n")
		b.WriteString("\tif (s.len + 1 > ncap) {\n")
		b.WriteString("\t\tncap = ncap < 4 ? 4 : ncap * 2;\n")
		b.WriteString("\t\twhile (ncap < s.len + 1) ncap *= 2;\n")
		b.WriteString("\t\tint32_t *nd = (int32_t *)aram_arena_alloc((size_t)ncap * sizeof(int32_t));\n")
	...
	if e.needUTF8 {
		b.WriteString("static int64_t aram_utf8_next(const char *s, int64_t *ip) {\n")
	...
	if e.needRuneStr {
		e.needArena = true
		b.WriteString("static const char *aram_rune_str(int64_t r) {\n")
		...
		b.WriteString("\tchar *out = (char *)aram_arena_alloc((size_t)n + 1);\n")
		b.WriteString("\tmemcpy(out, tmp, (size_t)n + 1);\n")
		b.WriteString("\treturn out;\n")
		b.WriteString("}\n")
	}
	if e.needStrBytes {
		b.WriteString("static const char *aram_bytes_str(aram_slice_u8 s) {\n")
		b.WriteString("\tchar *r = (char *)aram_arena_alloc((size_t)s.len + 1);\n")
		b.WriteString("\tif (s.len && s.data) memcpy(r, s.data, (size_t)s.len);\n")
		b.WriteString("\tr[s.len] = '\\0';\n\treturn r;\n}\n")
		b.WriteString("static const char *aram_runes_str(aram_slice_i32 s) {\n")
		b.WriteString("\tsize_t cap = (size_t)s.len * 4 + 1;\n")
		b.WriteString("\tchar *r = (char *)aram_arena_alloc(cap);\n")
		b.WriteString("\tsize_t n = 0;\n")
		b.WriteString("\tfor (int64_t i = 0; i < s.len; i++) {\n")
		b.WriteString("\t\tconst char *p = aram_rune_str((int64_t)s.data[i]);\n")
		b.WriteString("\t\tsize_t pn = strlen(p);\n")
		b.WriteString("\t\tmemcpy(r + n, p, pn);\n")
		b.WriteString("\t\tn += pn;\n")
		b.WriteString("\t}\n")
		b.WriteString("\tr[n] = '\\0';\n\treturn r;\n}\n")
		b.WriteString("static aram_slice_u8 aram_str_bytes(const char *s) {\n")
		b.WriteString("\tsize_t n = s ? strlen(s) : 0;\n")
		b.WriteString("\tif (n == 0) return (aram_slice_u8){0};\n")
		b.WriteString("\tuint8_t *d = (uint8_t *)aram_arena_alloc(n);\n")
		b.WriteString("\tmemcpy(d, s, n);\n")
		b.WriteString("\treturn (aram_slice_u8){ d, (int64_t)n, (int64_t)n };\n}\n")
		b.WriteString("static aram_slice_i32 aram_str_runes(const char *s) {\n")
		b.WriteString("\tif (!s || !*s) return (aram_slice_i32){0};\n")
		b.WriteString("\tint64_t n = 0, i = 0;\n")
		b.WriteString("\twhile (s[i]) { aram_utf8_next(s, &i); n++; }\n")
		b.WriteString("\tint32_t *d = (int32_t *)aram_arena_alloc((size_t)n * sizeof(int32_t));\n")
		b.WriteString("\ti = 0;\n")
	...
	if e.needDefer {
		...
		b.WriteString("\tstruct aram_defer_frame *next;\n")
		b.WriteString("} aram_defer_frame;\n\n")
	}
...
func (e *emitter) sliceCName(t check.Type) string {
	...
	case check.TypeSliceInt:
		return "aram_slice_i64"
	case check.TypeSliceBool:
		return "aram_slice_bool"
	case check.TypeSliceStr:
		return "aram_slice_str"
	case check.TypeSliceFloat:
		return "aram_slice_f64"
	case check.TypeSliceByte:
		return "aram_slice_u8"
	case check.TypeSliceRune:
		return "aram_slice_i32"
	default:
		return fmt.Sprintf("aram_slice_t%d", int(t))
	}
}
...
func (e *emitter) sliceGetName(t check.Type) string {
	switch t {
	case check.TypeSliceInt:
		return "aram_slice_get_i64"
	case check.TypeSliceBool:
		return "aram_slice_get_bool"
	case check.TypeSliceStr:
		return "aram_slice_get_str"
	case check.TypeSliceFloat:
		return "aram_slice_get_f64"
	case check.TypeSliceByte:
		return "aram_slice_get_u8"
	case check.TypeSliceRune:
		return "aram_slice_get_i32"
	default:
		return fmt.Sprintf("aram_slice_get_t%d", int(t))
	}
}
...
func (e *emitter) sliceSetName(t check.Type) string {
	switch t {
	case check.TypeSliceInt:
		return "aram_slice_set_i64"
	case check.TypeSliceBool:
		return "aram_slice_set_bool"
	case check.TypeSliceStr:
		return "aram_slice_set_str"
	case check.TypeSliceFloat:
		return "aram_slice_set_f64"
	case check.TypeSliceByte:
		return "aram_slice_set_u8"
	case check.TypeSliceRune:
		return "aram_slice_set_i32"
	default:
		return fmt.Sprintf("aram_slice_set_t%d", int(t))
	}
}
...
func (e *emitter) sliceSubName(t check.Type) string {
	...
	case check.TypeSliceRune:
		return "aram_sub_i32"
	default:
		return fmt.Sprintf("aram_sub_t%d", int(t))
	}
}
...
func (e *emitter) sliceSub3Name(t check.Type) string {
	switch t {
	case check.TypeSliceInt:
		return "aram_sub3_i64"
	case check.TypeSliceBool:
		return "aram_sub3_bool"
	case check.TypeSliceStr:
		return "aram_sub3_str"
	case check.TypeSliceFloat:
		return "aram_sub3_f64"
	case check.TypeSliceByte:
		return "aram_sub3_u8"
	case check.TypeSliceRune:
		return "aram_sub3_i32"
	default:
		return fmt.Sprintf("aram_sub3_t%d", int(t))
	}
}
...
func (e *emitter) sliceAppendName(t check.Type) string {
	switch t {
	case check.TypeSliceInt:
		return "aram_append_i64"
	case check.TypeSliceBool:
		return "aram_append_bool"
	case check.TypeSliceStr:
		return "aram_append_str"
	case check.TypeSliceFloat:
		return "aram_append_f64"
	case check.TypeSliceByte:
		return "aram_append_u8"
	case check.TypeSliceRune:
		return "aram_append_i32"
	default:
		return fmt.Sprintf("aram_append_t%d", int(t))
	}
}
...
func (e *emitter) writeNestedSliceAppend(b *strings.Builder) {
	for _, t := range e.nestedSliceOrder() {
		...
		b.WriteString("\t\tncap = ncap < 4 ? 4 : ncap * 2;\n")
		b.WriteString("\t\twhile (ncap < s.len + 1) ncap *= 2;\n")
		fmt.Fprintf(b, "\t\t%s *nd = (%s *)aram_arena_alloc((size_t)ncap * sizeof(%s));\n", en, en, en)
...
func (e *emitter) makeName(t check.Type) string {
	switch t {
	case check.TypeSliceInt:
		return "aram_make_i64"
	case check.TypeSliceBool:
		return "aram_make_bool"
	case check.TypeSliceStr:
		return "aram_make_str"
	case check.TypeSliceFloat:
		return "aram_make_f64"
	case check.TypeSliceByte:
		return "aram_make_u8"
	case check.TypeSliceRune:
		return "aram_make_i32"
	default:
		return fmt.Sprintf("aram_make_t%d", int(t))
	}
}
...
func (e *emitter) copyName(t check.Type) string {
	switch t {
	case check.TypeSliceInt:
		return "aram_copy_i64"
	case check.TypeSliceBool:
		return "aram_copy_bool"
	case check.TypeSliceStr:
		return "aram_copy_str"
	case check.TypeSliceFloat:
		return "aram_copy_f64"
	case check.TypeSliceByte:
		return "aram_copy_u8"
	case check.TypeSliceRune:
		return "aram_copy_i32"
	default:
		return fmt.Sprintf("aram_copy_t%d", int(t))
	}
}
...
func (e *emitter) writeOneMake(b *strings.Builder, t check.Type) {
	...
	fmt.Fprintf(b, "\t%s *d = (%s *)aram_arena_alloc((size_t)cap * sizeof(%s));\n", en, en, en)
...
func (e *emitter) eqFuncName(si *check.StructInfo) string {
	return "aram_eq_" + cPkgIdent(si.Pkg, si.Name)
}
...
func (e *emitter) printFuncName(si *check.StructInfo) string {
	return "aram_print_" + cPkgIdent(si.Pkg, si.Name)
}
...
func (e *emitter) writeFieldPrint(b *strings.Builder, access string, t check.Type) {
	...
	case t == check.TypeInt, t == check.TypeByte, t == check.TypeRune:
		b.WriteString("\taram_print_int_n((int64_t)(")
		b.WriteString(access)
		b.WriteString("));\n")
	case t == check.TypeFloat:
		b.WriteString("\taram_print_float_n(")
		b.WriteString(access)
		b.WriteString(");\n")
	case t == check.TypeBool:
		b.WriteString("\taram_print_bool_n(")
		b.WriteString(access)
		b.WriteString(");\n")
	case t == check.TypeString:
	...
	default:
		// pointer
		b.WriteString("\taram_print_ptr_n((const void *)(")
		b.WriteString(access)
		b.WriteString("));\n")
	}
}
...
func (e *emitter) writeSliceFieldPrint(b *strings.Builder, access string, t check.Type) {
	...
	case elem == check.TypeBool:
		fmt.Fprintf(b, "\t\taram_print_bool_n(%s(%s, __i));\n", e.sliceGetName(t), access)
	case elem == check.TypeString:
		fmt.Fprintf(b, "\t\taram_print_quoted_n(%s(%s, __i));\n", e.sliceGetName(t), access)
	...
	default:
		if e.info != nil {
			if _, ok := e.info.PtrElem[elem]; ok {
				fmt.Fprintf(b, "\t\taram_print_ptr_n(%s(%s, __i));\n", e.sliceGetName(t), access)
				break
			}
		}
		fmt.Fprintf(b, "\t\taram_print_int_n(%s(%s, __i));\n", e.sliceGetName(t), access)
	}
	b.WriteString("\t}\n")
	b.WriteString("\tprintf(\"]\");\n")
}
...
func (e *emitter) arrayCName(t check.Type) string {
	ai := e.info.Arrays[t]
	return fmt.Sprintf("aram_arr_%s_%d", e.retTag(ai.Elem), ai.Len)
}

func (e *emitter) mapCName(t check.Type) string {
	return fmt.Sprintf("aram_map_t%d", int(t))
}

func (e *emitter) mapTabName(t check.Type) string {
	return fmt.Sprintf("aram_map_tab_t%d", int(t))
}

func (e *emitter) mapEntName(t check.Type) string {
	return fmt.Sprintf("aram_map_ent_t%d", int(t))
}
...
func (e *emitter) mapKeyHashName(t check.Type) string {
	...
	if e.info != nil {
		if si, ok := e.info.Structs[t]; ok {
			return "aram_hash_" + cPkgIdent(si.Pkg, si.Name)
		}
	}
	return fmt.Sprintf("aram_hash_t%d", int(t))
}
...
func (e *emitter) mapKeyEqName(t check.Type) string {
	...
	return fmt.Sprintf("aram_eq_t%d", int(t))
}
...
func (e *emitter) writeMapFuncs(b *strings.Builder) {
	...
	e.needMap = true
	e.needArena = true
	b.WriteString("static uint64_t aram_map_hash_str(const char *s) {\n")
	b.WriteString("\tuint64_t h = 14695981039346656037ULL;\n")
	...
	b.WriteString("\treturn h;\n")
	b.WriteString("}\n")
	b.WriteString("static uint64_t aram_map_hash_f64(double x) {\n")
	...
	for _, t := range keys {
		...
		b.WriteString("\t}\n")
		fmt.Fprintf(b, "\t%s *tab = (%s *)aram_arena_alloc(sizeof(%s));\n", tn, tn, tn)
		b.WriteString("\ttab->len = 0; tab->cap = cap;\n")
		fmt.Fprintf(b, "\ttab->entries = (%s *)aram_arena_alloc((size_t)cap * sizeof(%s));\n", en, en)
		...
		fmt.Fprintf(b, "\t%s *neu = (%s *)aram_arena_alloc((size_t)ncap * sizeof(%s));\n", en, en, en)
...
func (e *emitter) writeDeferEpilogue(b *strings.Builder, fn *ast.FuncDecl) {
	b.WriteString("__aram_epilogue:;\n")
	if e.fnHasDefer {
		b.WriteString("\twhile (_defers) {\n")
		b.WriteString("\t\taram_defer_frame *_df = _defers;\n")
		b.WriteString("\t\t_defers = _df->next;\n")
		if e.needPanic {
			b.WriteString("\t\taram_defer_pending = 1;\n")
...
func (e *emitter) writeDefer(b *strings.Builder, s *ast.DeferStmt, level int) {
	...
	id := e.deferID
	e.deferID++
	thunk := fmt.Sprintf("aram_dthunk_%d", id)
	...
	if len(fields) == 0 {
		indent(b, level+1)
		b.WriteString("aram_defer_frame *_df = (aram_defer_frame *)aram_arena_alloc(sizeof(aram_defer_frame));\n")
		indent(b, level+1)
		...
		b.WriteString("} *_dc = aram_arena_alloc(sizeof(*_dc));\n")
		for _, f := range fields {
			indent(b, level+1)
...
func (e *emitter) writeDeferredCall(
	...
	if builtinName != "" {
		...
		case "பதிப்பி":
			...
			switch t {
			case check.TypeString:
				fmt.Fprintf(b, "aram_print_str(%s)", fieldRef(0))
			case check.TypeBool:
				fmt.Fprintf(b, "aram_print_bool(%s)", fieldRef(0))
			case check.TypeFloat:
				fmt.Fprintf(b, "aram_print_float(%s)", fieldRef(0))
			default:
				fmt.Fprintf(b, "aram_print_int(%s)", fieldRef(0))
			}
			return
		case "நீக்கு":
			if len(call.Args) >= 1 {
		...
		case "மீள்":
			e.needPanic = true
			b.WriteString("(void)aram_recover()")
			return
...
func (e *emitter) writeReturnWithDefer(b *strings.Builder, s *ast.ReturnStmt, level int) {
	...
	indent(b, level)
	b.WriteString("goto __aram_epilogue;\n")
}
...
func (e *emitter) writeSwitch(b *strings.Builder, s *ast.SwitchStmt, level int) {
	...
	if s.Tag != nil {
		e.swID++
		tmp = fmt.Sprintf("__aram_sw_%d", e.swID)
		tagT = e.typeOf(s.Tag)
		indent(b, level+1)
...
func (e *emitter) writeRangeChan(b *strings.Builder, s *ast.RangeStmt, level int) {
	...
	b.WriteString(";\n")
	indent(b, level+2)
	fmt.Fprintf(b, "int %s = aram_chan_recv(%s, &%s);\n", ok, ch, val)
	indent(b, level+2)
...
func (e *emitter) writeConversion(b *strings.Builder, call *ast.CallExpr) {
	...
	if len(call.Args) == 1 {
		...
		if du == check.TypeString && (src == check.TypeInt || src == check.TypeRune || src == check.TypeByte) {
			e.needRuneStr = true
			e.needArena = true
			b.WriteString("aram_rune_str((int64_t)(")
...
func (e *emitter) writePrint(b *strings.Builder, arg ast.Expr) {
	...
	switch t {
	case check.TypeString:
		b.WriteString("aram_print_str(")
		e.writeExpr(b, arg)
		b.WriteByte(')')
	case check.TypeBool:
		b.WriteString("aram_print_bool(")
		e.writeExpr(b, arg)
		b.WriteByte(')')
	case check.TypeFloat:
		b.WriteString("aram_print_float(")
		e.writeExpr(b, arg)
		b.WriteByte(')')
	case check.TypeByte, check.TypeRune:
		b.WriteString("aram_print_int((int64_t)(")
		e.writeExpr(b, arg)
		b.WriteString("))")
	default:
		switch arg := arg.(type) {
