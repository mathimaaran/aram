...
func (e *emitter) writeChanRuntime(b *strings.Builder) {
	...
	b.WriteString("\taram_chan *c = (aram_chan *)calloc(1, sizeof(aram_chan));\n")
	...
	b.WriteString("\t\tc->buf = (char *)calloc((size_t)c->cap, (size_t)elem_size);\n")
	b.WriteString("\t\tif (!c->buf) abort();\n")
	b.WriteString("\t} else {\n")
	b.WriteString("\t\tc->slot = (char *)calloc(1, (size_t)elem_size);\n")
	...
	b.WriteString("\tif (c->closed) { pthread_mutex_unlock(&c->mu); abort(); }\n")
	b.WriteString("\tif (c->cap == 0) {\n")
	b.WriteString("\t\tif (c->has_slot) { pthread_mutex_unlock(&c->mu); return 0; }\n")
	...
	b.WriteString("\t\t\t\tpthread_cond_signal(&c->recv_cv);\n")
	b.WriteString("\t\t\t\taram_sel_kick();\n")
	b.WriteString("\t\t\t\twhile (c->has_slot && !c->closed) pthread_cond_wait(&c->send_cv, &c->mu);\n")
	b.WriteString("\t\t\t\tpthread_mutex_unlock(&c->mu);\n")
	...
	b.WriteString("\t\t\treturn;\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\tpthread_cond_wait(&c->send_cv, &c->mu);\n")
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
	...
	b.WriteString("\t\t\treturn 0;\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\tpthread_cond_wait(&c->recv_cv, &c->mu);\n")
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
	b.WriteString("typedef struct { aram_chan *ch; int is_send; void *data; int *ok; } aram_sel_case;\n")
	// has_def: if set and no case ready, return n (default sentinel). Else park on aram_sel_cv.
	...
	b.WriteString("\t\tpthread_mutex_lock(&aram_sel_mu);\n")
	b.WriteString("\t\tpthread_cond_wait(&aram_sel_cv, &aram_sel_mu);\n")
	b.WriteString("\t\tpthread_mutex_unlock(&aram_sel_mu);\n")
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
}
