...
func (e *emitter) writeChanRuntime(b *strings.Builder) {
	...
	if e.needGo {
		b.WriteString("static pthread_mutex_t aram_go_mu = PTHREAD_MUTEX_INITIALIZER;\n")
		b.WriteString("static pthread_t *aram_go_threads;\n")
		b.WriteString("static size_t aram_go_n, aram_go_cap;\n")
		b.WriteString("static void aram_go(void *(*fn)(void *), void *arg) {\n")
		b.WriteString("\tpthread_t th;\n")
		b.WriteString("\tif (pthread_create(&th, NULL, fn, arg) != 0) abort();\n")
		b.WriteString("\tpthread_mutex_lock(&aram_go_mu);\n")
		b.WriteString("\tif (aram_go_n + 1 > aram_go_cap) {\n")
		b.WriteString("\t\tsize_t nc = aram_go_cap ? aram_go_cap * 2 : 8;\n")
		b.WriteString("\t\taram_go_threads = (pthread_t *)realloc(aram_go_threads, nc * sizeof(pthread_t));\n")
		b.WriteString("\t\tif (!aram_go_threads) abort();\n")
		b.WriteString("\t\taram_go_cap = nc;\n")
		b.WriteString("\t}\n")
		b.WriteString("\taram_go_threads[aram_go_n++] = th;\n")
		b.WriteString("\tpthread_mutex_unlock(&aram_go_mu);\n")
		b.WriteString("}\n")
		b.WriteString("static void aram_go_wait_all(void) {\n")
		b.WriteString("\tpthread_mutex_lock(&aram_go_mu);\n")
		b.WriteString("\tsize_t n = aram_go_n;\n")
		b.WriteString("\tpthread_t *ts = aram_go_threads;\n")
		b.WriteString("\taram_go_n = 0;\n")
		b.WriteString("\tpthread_mutex_unlock(&aram_go_mu);\n")
		b.WriteString("\tfor (size_t i = 0; i < n; i++) pthread_join(ts[i], NULL);\n")
		b.WriteString("\tfree(ts);\n")
		b.WriteString("\taram_go_threads = NULL; aram_go_cap = 0;\n")
		b.WriteString("}\n\n")
	}
	if !e.needChan {
		return
	...
	b.WriteString("static pthread_mutex_t aram_sel_mu = PTHREAD_MUTEX_INITIALIZER;\n")
	b.WriteString("static pthread_cond_t aram_sel_cv = PTHREAD_COND_INITIALIZER;\n")
	b.WriteString("static unsigned aram_sel_gen;\n")
	b.WriteString("static void aram_sel_kick(void) {\n")
	b.WriteString("\tpthread_mutex_lock(&aram_sel_mu);\n")
	b.WriteString("\taram_sel_gen++;\n")
	b.WriteString("\tpthread_cond_broadcast(&aram_sel_cv);\n")
	b.WriteString("\tpthread_mutex_unlock(&aram_sel_mu);\n")
	b.WriteString("}\n\n")
	b.WriteString("typedef struct aram_chan {\n")
	...
	b.WriteString("static aram_chan *aram_chan_make(int elem_size, int64_t cap) {\n")
	b.WriteString("\tif (elem_size <= 0 || cap < 0) abort();\n")
	b.WriteString("\taram_chan *c = (aram_chan *)aram_alloc(sizeof(aram_chan));\n")
	...
	b.WriteString("\t\tc->buf = (char *)aram_alloc((size_t)c->cap * (size_t)elem_size);\n")
	b.WriteString("\t\tif (!c->buf) abort();\n")
	b.WriteString("\t} else {\n")
	b.WriteString("\t\tc->slot = (char *)aram_alloc((size_t)elem_size);\n")
	b.WriteString("\t\tif (!c->slot) abort();\n")
	b.WriteString("\t}\n")
	b.WriteString("\treturn c;\n")
	b.WriteString("}\n\n")
	b.WriteString("static void aram_chan_close(aram_chan *c) {\n")
	b.WriteString("\tif (!c) abort();\n")
	...
	b.WriteString("\taram_sel_kick();\n")
	b.WriteString("}\n\n")
	b.WriteString("static int aram_chan_try_send(aram_chan *c, const void *data) {\n")
	b.WriteString("\tif (!c) abort();\n")
	b.WriteString("\tpthread_mutex_lock(&c->mu);\n")
	b.WriteString("\tif (c->closed) { pthread_mutex_unlock(&c->mu); abort(); }\n")
	b.WriteString("\tif (c->cap == 0) {\n")
	...
	b.WriteString("\treturn 1;\n")
	b.WriteString("}\n\n")
	b.WriteString("static void aram_chan_send(aram_chan *c, const void *data) {\n")
	b.WriteString("\tif (!c) abort();\n")
	b.WriteString("\tpthread_mutex_lock(&c->mu);\n")
	b.WriteString("\tfor (;;) {\n")
	b.WriteString("\t\tif (c->closed) { pthread_mutex_unlock(&c->mu); abort(); }\n")
	...
	b.WriteString("\t\t\t\tpthread_cond_signal(&c->recv_cv);\n")
	b.WriteString("\t\t\t\taram_sel_kick();\n")
	b.WriteString("\t\t\t\twhile (c->has_slot && !c->closed) ARAM_GC_COND_WAIT(&c->send_cv, &c->mu);\n")
	b.WriteString("\t\t\t\tpthread_mutex_unlock(&c->mu);\n")
	...
	b.WriteString("\t\t\treturn;\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\tARAM_GC_COND_WAIT(&c->send_cv, &c->mu);\n")
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
	b.WriteString("static int aram_chan_try_recv(aram_chan *c, void *out, int *ok) {\n")
	b.WriteString("\tif (!c) abort();\n")
	b.WriteString("\tpthread_mutex_lock(&c->mu);\n")
	b.WriteString("\tif (c->cap == 0) {\n")
	b.WriteString("\t\tif (c->has_slot) {\n")
	...
	b.WriteString("\treturn 0;\n")
	b.WriteString("}\n\n")
	b.WriteString("static int aram_chan_recv(aram_chan *c, void *out) {\n")
	...
	b.WriteString("\t\tpthread_mutex_lock(&c->mu);\n")
	b.WriteString("\t\tif (c->cap == 0) {\n")
	b.WriteString("\t\t\tif (!c->has_slot && !c->closed) ARAM_GC_COND_WAIT(&c->recv_cv, &c->mu);\n")
	b.WriteString("\t\t} else {\n")
	b.WriteString("\t\t\tif (c->len == 0 && !c->closed) ARAM_GC_COND_WAIT(&c->recv_cv, &c->mu);\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t\tc->recv_waiters--;\n")
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
	b.WriteString("typedef struct { aram_chan *ch; int is_send; void *data; int *ok; } aram_sel_case;\n")
	b.WriteString("static int aram_select_try(aram_sel_case *cs, int n) {\n")
	b.WriteString("\tfor (int i = 0; i < n; i++) {\n")
	b.WriteString("\t\tif (cs[i].is_send) {\n")
	b.WriteString("\t\t\tif (aram_chan_try_send(cs[i].ch, cs[i].data)) return i;\n")
	b.WriteString("\t\t} else {\n")
	b.WriteString("\t\t\tif (aram_chan_try_recv(cs[i].ch, cs[i].data, cs[i].ok)) return i;\n")
	b.WriteString("\t\t}\n")
	b.WriteString("\t}\n")
	b.WriteString("\treturn -1;\n")
	b.WriteString("}\n")
	b.WriteString("static void aram_select_arm(aram_sel_case *cs, int n) {\n")
	b.WriteString("\tfor (int i = 0; i < n; i++) {\n")
	...
	b.WriteString("\t}\n")
	b.WriteString("}\n")
	b.WriteString("static void aram_select_disarm(aram_sel_case *cs, int n) {\n")
	b.WriteString("\tfor (int i = 0; i < n; i++) {\n")
	b.WriteString("\t\tif (cs[i].is_send || !cs[i].ch) continue;\n")
	b.WriteString("\t\tpthread_mutex_lock(&cs[i].ch->mu);\n")
	...
	// Else arm recv waiters, re-try, then park on aram_sel_cv (gen avoids lost wakeup).
	b.WriteString("static int aram_select(aram_sel_case *cs, int n, int has_def) {\n")
	b.WriteString("\tfor (;;) {\n")
	b.WriteString("\t\tint i = aram_select_try(cs, n);\n")
	b.WriteString("\t\tif (i >= 0) return i;\n")
	b.WriteString("\t\tif (has_def) return n;\n")
	b.WriteString("\t\taram_select_arm(cs, n);\n")
	b.WriteString("\t\taram_sel_kick();\n")
	b.WriteString("\t\tpthread_mutex_lock(&aram_sel_mu);\n")
	b.WriteString("\t\tunsigned gen = aram_sel_gen;\n")
	b.WriteString("\t\tpthread_mutex_unlock(&aram_sel_mu);\n")
	b.WriteString("\t\ti = aram_select_try(cs, n);\n")
	b.WriteString("\t\tif (i >= 0) { aram_select_disarm(cs, n); return i; }\n")
	b.WriteString("\t\tpthread_mutex_lock(&aram_sel_mu);\n")
	b.WriteString("\t\twhile (aram_sel_gen == gen) ARAM_GC_COND_WAIT(&aram_sel_cv, &aram_sel_mu);\n")
	b.WriteString("\t\tpthread_mutex_unlock(&aram_sel_mu);\n")
	b.WriteString("\t\taram_select_disarm(cs, n);\n")
	b.WriteString("\t}\n")
	b.WriteString("}\n\n")
}
...
func (e *emitter) writeRecvCommaOk(b *strings.Builder, u *ast.UnaryExpr, valName, okName string) {
	...
	elemCT := e.cTypeFrom(ci.Elem)
	fmt.Fprintf(b, "%s %s;\n", elemCT, valName)
	fmt.Fprintf(b, "int %s = aram_chan_recv(", okName)
	e.writeExpr(b, u.X)
	fmt.Fprintf(b, ", &%s);\n", valName)
}
...
func (e *emitter) writeGoStmt(b *strings.Builder, s *ast.GoStmt, level int) {
	...
	envName := fmt.Sprintf("aram_go_env_%d", id)
	trampName := fmt.Sprintf("aram_go_tramp_%d", id)
	...
	tb.WriteString("\taram_thread_register();\n")
	fmt.Fprintf(tb, "\t%s *e = (%s *)p;\n", envName, envName)
	if e.needPanic {
		tb.WriteString("\taram_panic_push(\"இழை\");\n")
	}
	tb.WriteByte('\t')
	if isFnVal {
	...
	if e.needPanic {
		tb.WriteString("\taram_panic_pop();\n")
	}
	tb.WriteString("\taram_thread_unregister();\n")
	tb.WriteString("\treturn NULL;\n}\n")

	indent(b, level)
	fmt.Fprintf(b, "%s *__goe_%d = (%s *)aram_arena_alloc(sizeof(%s));\n", envName, id, envName, envName)
	if isFnVal {
		indent(b, level)
	...
	indent(b, level)
	fmt.Fprintf(b, "aram_go(%s, __goe_%d);\n", trampName, id)
}
...
func (e *emitter) writeSelect(b *strings.Builder, s *ast.SelectStmt, level int) {
	...
	indent(b, level)
	fmt.Fprintf(b, "aram_sel_case __sc_%d[%d];\n", id, max(nComm, 1))
	commI := 0
	for i, c := range cases {
		if c.isDefault {
			continue
	...
	fmt.Fprintf(b, "int __si_%d = aram_select(__sc_%d, %d, %d);\n", id, id, nComm, hasDefI)
	indent(b, level)
